...
const persistTracklist = (): void => {
  const tracklist = getItemFromLocalStorage<TrackModel[]>('tracklist');

  if (tracklist) {
    return;
  }

  setItemToLocalStorage('tracklist', tracks);
};
...
export const getFavorites = (): PlaylistModel => {
  return {
    ...
    tracks: [],
    coverUrl: 'favorites-placeholder.jpg',
    user: 'MonkeyBoy',
  };
};

export const getInitialPlaylists = (): {
  [x: string]: PlaylistModel;
} => {
...
function getTracks(searchField: keyof TrackModel, searchValue: string): TrackModel[] {
...
export const getRandomTracks = (tracks: TrackModel[]): TrackModel[] => {
