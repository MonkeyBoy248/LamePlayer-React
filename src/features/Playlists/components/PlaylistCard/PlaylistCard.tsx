    ...
    <li className={styles.playlistCard}>
      <Link className={styles.playlistCard__inner} to={`/playlist/${playlist.id}`}>
        <div className={styles.playlistCard__coverWrapper}>
          <img className={styles.playlistCard__cover} src={`/images/covers/${playlist.coverUrl}`} alt={`${playlist.title} cover`} />
        </div>
      </Link>
      <Link to={`/playlist/${playlist.id}`} className={`${styles.playlistCard__title} _itemTitle`}>{playlist.title}</Link>
